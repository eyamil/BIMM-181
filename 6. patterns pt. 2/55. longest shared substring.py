...
# Given two genome strings, find the longest substring that they both share.
...
def main():
    ...
    shared_substrings = list(find_shared_substr(tree, 0, color_ds, ""))
    longest_repeat = max(shared_substrings, key = lambda x : len(x))
...
def find_shared_substr(tree, node, color_ds, base_str):
    if (len(tree[node]) == 0) or not (color_ds[node][0] and color_ds[node][1]):
        return(set())
    else:
        ...
        for edge_label in outgoing_edges:
            repeat_set = repeat_set.union(find_shared_substr(tree, tree[node][edge_label], color_ds, base_str + edge_label))
        return(repeat_set)
