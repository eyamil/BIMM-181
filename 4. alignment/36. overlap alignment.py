...
# Given two strings, compute the best overlap alignment between the two.
# The match score is +1, and both mismatch and indel penalties are -2.
...
def do_dp(str1, str2, indel_penalty):
    ...
    for j in range(len(str2) + 1):
        if dp_table[len(str1)][j][0] > dp_table[len(str1)][len(str2)][0]:
            dp_table[len(str1)][len(str2)] = (dp_table[len(str1)][j][0], False, (len(str1), j))
    return(dp_table)
