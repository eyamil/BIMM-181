...
# Given two strings, find the best global alignment between them using the BLOSUM62
    ...
    for i in range(len(str1) + 1):
        for j in range(len(str2) + 1):
            ...
            if i > 0 and j > 0:
                ...
                entry = (align_score + dp_table[prev[0]][prev[1]][0], prev)
                possible_values.append(entry)
            if i > 0:
                prev = (i - 1, j)
                entry = (indel_penalty + dp_table[prev[0]][prev[1]][0], prev)
                possible_values.append(entry)
            if j > 0:
                prev = (i, j - 1)
                entry = (indel_penalty + dp_table[prev[0]][prev[1]][0], prev)
